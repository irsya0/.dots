...
-- Src: https://github.com/nvim-telescope/telescope.nvim/blob/master/lua/telescope/builtin/internal.luater
        ...
        attach_mappings = function(prompt_bufnr)
            actions.select_default:replace(function()
                ...
                if vim.fn.bufwinid(entry.bufnr) == -1 then
                    local termopts = vim.g.terms[tostring(entry.bufnr)]
                    if termopts then
                        require("ui.components.term").display(termopts)
                    else
